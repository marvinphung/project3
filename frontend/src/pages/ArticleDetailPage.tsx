  ...
  return (
    <main className="max-w-[900px] mx-auto px-4 sm:px-6 py-8">
      ...
      <article>
        ...
        <p className="mt-4 text-sm text-[#6B7280]">Xuất bản {new Date(article.published_at).toLocaleString('vi-VN')}</p>
      ...
      <section className="mt-10">
        ...
        <div className="space-y-3">
          {sources.data?.map((source) => (
            <a key={source.source_id} href={source.source_url} target="_blank" rel="noreferrer" className="block rounded-lg border border-[#E5E7EB] bg-white p-3 hover:border-[#78A83D]/50">
              <strong className="text-sm text-[#111827]">{source.source_name}</strong>
              <p className="mt-1 text-xs text-[#6B7280]">
                Độ tin cậy tier {source.reliability_tier}{source.published_at ? ` · ${new Date(source.published_at).toLocaleString('vi-VN')}` : ''}
              </p>
            </a>
          ))}
        </div>
