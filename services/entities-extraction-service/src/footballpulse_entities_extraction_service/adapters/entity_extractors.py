...
LOGGER = logging.getLogger("footballpulse.entities.entity_model")
...
def _load_gliner2(model_id: str, device: str = "cpu") -> Gliner2Model:
    try:
        ...
        raise RuntimeError(
            "GLiNER2 runtime is unavailable; install the entities-extraction-service model extra"
        ) from error
...
class GlinerEntityExtractor:
    ...
    def extract(
        ...
        *,
        ...
        if model is None:
            with self._lock:
                if self._model is None:
                    ...
                    log_event(LOGGER, "entity_model_loading", model=self._model_id, device=self._device)
        ...
        active_schema = {label.value: self._schema.get(label.value, label.value) for label in labels}
        ...
        return sorted(spans, key=lambda prediction: (prediction.start, prediction.end, prediction.label.value))
    ...
    def extract(
        ...
        *,
        ...
        for rule in self._rules:
            ...
            while start >= 0:
                predictions.append(ModelSpan(rule.text, rule.label, start, start + len(rule.text), rule.score))
                start = text.find(rule.text, start + len(rule.text))
        return sorted(predictions, key=lambda prediction: (prediction.start, prediction.end, prediction.label.value))
...
class CatalogAliasEntityExtractor:
    ...
    def __init__(self, *, rules: tuple[CatalogEntityRule, ...]) -> None:
        self._rules = tuple(sorted(rules, key=lambda rule: (-len(rule.alias), rule.alias.casefold())))
        ...
        *,
        ...
        for rule in self._rules:
            ...
            for match in pattern.finditer(text):
                predictions.append(ModelSpan(match.group(), rule.label, match.start(), match.end(), 1.0))
        return sorted(predictions, key=lambda prediction: (prediction.start, prediction.end, prediction.label.value))
